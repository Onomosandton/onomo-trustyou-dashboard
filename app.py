...
# 2. Secure Firebase Live Data Fetcher via REST API
def fetch_live_firebase_data():
    try:
        # Pull configurations securely from Streamlit Secrets
        project_id = st.secrets["firebase"]["project_id"]
        
        # Target the exact path matching your React configuration
        ...
        response = requests.get(base_url, params={"pageSize": 300})
        ...
        for doc in documents:
            ...
            # Extract fields dynamically checking Firestore datatypes
        ...
        st.sidebar.error(f"Live Sync offline: {e}")
        return pd.DataFrame()

# 3. Base64 Asset Loader
...
if st.session_state.uploaded_file is None:
    ...
    with col2:
        ...
        st.markdown("<p style='text-align: center; color: #666; font-size: 1.1rem; margin-bottom: 30px;'>Drop your TrustYou review file here to instantly pull correlating floor actions directly from the server.</p>", unsafe_allow_html=True)
    ...
    try:
        ...
        # AUTOMATED STEP: Pull Live Firebase data directly down over HTTP pipeline
        fb_df = fetch_live_firebase_data()
        
        # 6. EXACT CROSS-PLATFORM CORRELATION MATHEMATICS
        def map_live_operational_state(ty_row, live_db):
            if live_db.empty: return "No Log Found"
            
            author_name = str(ty_row['Author name']).lower().strip()
            review_date = ty_row['Published date']
            
            if pd.isna(review_date) or author_name in ['nan', '']: return "No Log Found"
            
            # Narrow historical analysis window to tracking actions inside the week of departure
            ...
            for _, fb_row in active_window_logs.iterrows():
                ...
                # Check cross-reference matching on string intersections
            ...
            return "Blindspot (Missed)" if ty_row['Score'] < 80 else "No Log Found"
        ...
        # 8. VISUALIZATION OF INTERACTION IMPACT
        ...
        action_analysis = action_analysis[action_analysis['Operational_Status'] != 'No Log Found'].sort_values('Score', ascending=False)
        ...
        if not action_analysis.empty:
            ...
            st.info("System connected successfully. Awaiting operational matches across active time window profiles.")
        st.markdown("</div>", unsafe_allow_html=True)
        
        # 9. LIVE SERVICE FAILURE AUDIT GRID
        st.markdown("<div class='glass-container' style='margin-top: 40px;'>", unsafe_allow_html=True)
        ...
        st.error(f"Data pipeline compilation error: {e}")
